...
async function probeCpamc(options) {
	...
	if (!options.enabled || origin === void 0 || !options.managementKey) return [];
...
function volcWindows(body) {
	...
	for (const row of rows) {
		...
		if (percent !== void 0) windows.push({
			key,
			percent,
			resetsAt: iso(row.ResetTimestamp ?? row.resetTimestamp ?? row.ResetTime ?? row.resetTime ?? row.resetAt)
		});
	}
	return windows;
}
	...
	pollNow() {
		...
		const cycle = (async () => {
			try {
				...
				for (const id of [...this.snapshots.keys()]) if (!seen.has(id) && !id.startsWith("cpamc:") && !id.startsWith("volcano:")) this.snapshots.delete(id);
